...
import { DeviceGroup } from "./output/ton-contracts_DeviceGroup";

async function main() {
    console.log("Deploying...");
    const toProduction = process.argv.length === 3 && process.argv[2] === "mainnet";
    const init = await DeviceGroup.init();
    const prepare = await prepareTactDeployment({
        pkg: await readFile(resolve(__dirname, "output", "ton-contracts_DeviceGroup.pkg")),
